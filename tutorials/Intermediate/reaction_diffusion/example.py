...
def gaussian(x):
    return forcing * np.exp(- np.linalg.norm(x[:fe.dim] - pos)/(2 * (1/4)**2))
...
if plotting := True:
    ...
    fig_dir = Path("../../figures/Intermediate/reaction_diffusion/")
    ...
    pl.add_mesh(warped)
    pl.open_gif(fig_dir / "reaction_diffusion_movie.gif")
